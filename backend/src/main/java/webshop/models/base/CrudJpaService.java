...
public abstract class CrudJpaService<E extends BaseEntity<ID>,ID extends Serializable> implements CrudService<ID> {
    ...
    @Override
    public <T> T findById(ID id, Class<T> resultDtoClass) throws AppException {
    ...
    @Override
    public <T, U> T update(ID id, U object, Class<T> resultDtoClass) throws AppException {
        if(!repository.existsById(id))
            throw new AppException("Object not found in update method!");
        E entity = modelMapper.map(object,entityClass);
        entity.setId(id);
        entity = repository.saveAndFlush(entity);
        entityManager.refresh(entity);
    ...
    public E findEntityById(ID id) throws AppException {return repository.findById(id).orElseThrow(AppException::new);}

    @Override
    public void delete(ID id) throws AppException {
        if (!repository.existsById(id))
            throw new AppException("Object not found in delete method!");
